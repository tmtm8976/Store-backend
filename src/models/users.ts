...
export class userModel{
    ...
    async create(u: user): Promise<user> {
        try {
            const sql = `INSERT INTO users (first_name, last_name, password) VALUES($1, $2, $3) RETURNING first_name ,last_name, id ;`;
            const conn = await client.connect();
    ...
    async authenticate(id:string , password : string) :Promise<user|null> {
        ...
        try {
            ...
            if (result.rows.length != 0) {
                ...
                const validatePassword = bcrypt.compareSync(`${password}${PEPPER}`, hashPasword);
            

                if (validatePassword) {
                    const user_info = await conn.query("SELECT first_name, last_name, id FROM users WHERE id=($1);", [id]);
